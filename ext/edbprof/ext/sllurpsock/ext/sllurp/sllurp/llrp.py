...
class LLRPClient (LineReceiver):
    ...
    def __init__ (self, factory, duration=None, report_every_n_tags=None,
            antennas=(1,), tx_power=None, modulation=DEFAULT_MODULATION, tari=0,
    ...
    def parseCapabilities (self, capdict):
        ...
        logger.debug('requested tx_power: {} dBm'.format(
            self.tx_power if self.tx_power is not None else 'MAX'))
        ...
        if self.tx_power == None: # tx_power = None means max power
            self.tx_power = max(self.tx_power_table)
        else:
            # Find the preset that is equal to or lower than requested value
            for tx_power_idx in range(1, len(self.tx_power_table)):
                if self.tx_power_table[tx_power_idx] > self.tx_power:
                    self.tx_power_idx = tx_power_idx - 1
                    break
            if self.tx_power_idx is None: # no preset found => value out of range
                raise LLRPError('Invalid tx_power: requested={} dBm,' \
                        ' max_available={} dBm, min_available={} dBm'.format(
                            self.tx_power,
                            max(self.tx_power_table),
                            min(self.tx_power_table)))

            self.tx_power = self.tx_power_table[self.tx_power_idx]
        logger.debug('set tx_power: {} dBm (preset {})'.format(self.tx_power,
                     self.tx_power_idx))
    ...
    def getROSpec (self):
        ...
        self.rospec = LLRPROSpec(self, 1, duration_sec=self.duration,
                            ...
                            tx_power=self.tx_power_idx, antennas=self.antennas,
...
class LLRPClientFactory (ClientFactory):
    ...
    def politeShutdown (self, disconnect=False):
        """Stop inventory on all connected readers."""
        protoDeferreds = []
        for proto in self.protocols:
            protoDeferreds.append(proto.stopPolitely(disconnect))
