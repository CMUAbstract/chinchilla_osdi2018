...
void my_qsort(void *pbase, size_t total_elems, size_t size, __compar_fn_t cmp)
{
	...
	char static_alloca[10];
	char *pivot_buffer = &static_alloca[0];
...
int main(void) {
	...
	while (1) {
		...
		//array[count].distance = sqrt(array[count].x^2 + array[count].y^2 + array[count].z^2);
		LOG("test2\r\n");
